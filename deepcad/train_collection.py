...
if torch.cuda.is_available():
    print("train_collection.py")
    print('\033[1;31mGPU accessible. Use GPU for computation.\033[0m')
...
class training_class():
    ...
    def test(self, train_epoch, train_iteration):
        ...
        for iteration, (noise_patch, single_coordinate) in enumerate(testloader):
            ...
            real_A = noise_patch.type(torch.float32)
            real_A = Variable(real_A)
            fake_B = self.local_model(real_A)
...
def check_forkserver_support() -> bool:
    try:
        from os import fork as _
    except ImportError:
        return False
    else:
        del _
